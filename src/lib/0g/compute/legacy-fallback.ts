...
export function buildLabeledFallbackLegacy(params: {
  ...
  const story = `${ghost.name}'s World Cup was never just scores on a screen. It was ${ghost.team} in the blood, ${memories.length} evolution chapters deep, and a fan identity that grew louder with every reaction, comment, and heartbreak. From the first whistle to the final reflection, this legacy belongs on 0G: permanent, wallet-owned, and unmistakably yours. When live 0G Compute could not answer in time, this wrapped story was composed from your indexed journey so the unwrap ritual could continue.`;
  ...
  return {
    legacy: {
      ...
      dominantMood: ghost.evolutionScore >= 50 ? "fierce" : "hopeful",
      celebration: {
      ...
      fanIdentity: {
        title: `${ghost.name}, forever ${ghost.team}`,
        body: `This is not a highlight reel. It is your football identity, evolved through the tournament and verified on 0G.`,
      },
    },
    proof: {
